...
BeeCharts resolves series colors from `chartConfig` at runtime. Keys **must match** series `dataKey` / slice `nameKey` values.
...
BeeCharts resolves `var(--token)` to computed RGB before passing colors to ECharts, so CSS variables work in canvas rendering.
