...
name: nqchart-docs
description: >-
  Builds and extends the NQChart documentation site (Fumadocs + Next.js).
  ...
  in this repo. Pair with nqchart-dev for registry/engine work; do not use
...
# NQChart — documentation site skill

> Engine/registry: see [nqchart-dev](../nqchart-dev/SKILL.md).  
...
1. Add registry example in `src/registry/examples/ex-my-chart.tsx` (coordinate with nqchart-dev).
...
| `/.well-known/agent-skills/` | Synced from `skills/consumer/nqchart/` + maintainer skills |
| `/mcp` | `src/app/mcp/route.ts` |

Legacy `/.well-known/skills/*` paths redirect to `/.well-known/agent-skills/*` (see `next.config.ts`).

After editing consumer skill: `pnpm sync:skills` (updates `.agents/skills/nqchart/` and `public/.well-known/`).
