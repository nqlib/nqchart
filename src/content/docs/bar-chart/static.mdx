...
Bin numeric samples with `binForHistogram()` from [Chart recipes](/docs/chart-recipes), then render with `variant="histogram"` on `<NQBarChart />` (touching bins, square corners). Long bin labels truncate and hide overlaps when the plot is narrow — widen the card to show more ticks.
